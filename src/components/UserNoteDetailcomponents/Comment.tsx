...
const Comment: React.FC<CommentProps> = ({
    ...
    return (
        <div className="relative">
            {/* 토스트 */}
            <div
                className={`absolute -top-16 left-1/2 -translate-x-1/2 transition-all duration-300 ease-out z-50 ${toast.show
                ...
                <div className="flex-1">
                    ...
                    <div className="mt-3 flex items-center gap-3 ml-[10px]">
                        <button
                            ...
                            className={`inline-flex items-center gap-2 rounded-[6px] px-3 py-2 text-[13px] hover:bg-[#383E4D] mr-[5px] border-0 focus:outline-none focus:ring-0 transition-colors ${isLiked
                                    ? 'bg-[#6F4ACD] text-white'
                                    : 'bg-[#2F3440] text-[#C9CEDB]'
                                }`}
                        >
                            <ThumbsUpIcon className={`h-[20px] w-[20px] mr-[5px] ${isLiked ? 'text-white' : 'text-[#C9CEDB]'
                                }`} />
                            <span>{likes}</span>
                        </button>
